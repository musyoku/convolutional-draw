...
class GRUCore(chainer.Chain):
    ...
    def forward_onestep(self, prev_hg, prev_z, prev_r, downsampled_prev_r,
                        layernorm_step):
        lstm_in = cf.concat((prev_hg, prev_z, downsampled_prev_r), axis=1)
        ...
        lstm_x = cf.concat((prev_z, downsampled_prev_r), axis=1)
        lstm_h = cf.tanh(
            self.layernorm_tanh(
                self.gru_x(lstm_x) * reset_gate + self.gru_h(prev_hg),
                layernorm_step))
        next_h = update_gate * prev_hg + (1.0 - update_gate) * lstm_h
        next_r = cf.depth2space(self.conv_pixel_shuffle(next_h), r=2) + prev_r

        return next_h, next_r
